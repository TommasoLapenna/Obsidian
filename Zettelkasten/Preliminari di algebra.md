...
>**COSTRUZIONE DI CAMPO CON $q=p^r$ ELEMENTI**
	...
	**ESEMPIO**
	...
	Quindi il polinomio irriducibile è per forza $x^2+x+1$, verifica:
		1. Si elencano tutti i polinomi di grado $1:x,x+1$, si fanno tutti i prodotti possibili e di fa vedere che sono tutti polinomi riducibili di grado $2$:$(x+1)^2=x^2+1, \ x(x+1)=x^2+x, x^2\ni x^2+x+1$ segno che è riducibile.
		2. Se $f=x^2+x+1$ fosse riducibile allora esso ha almeno una radice in $\mathbb{F}_2$, ma non ne ha (se fosse riducibile, sarebbe scomponibile nella forma $f=(x+\alpha)(x+\beta), \ \alpha,\beta\in\mathbb{F}_2$, ma in $\mathbb{F}_2$ $x$ può essere solo $0$ o $1\rightarrow f(1)=1,\ f(0)=1$).
	Dunque $\mathbb{F}_4={\frac{\mathbb{F}_2[x]}{(x^2+x+1)}}$ è un campo con $4$ elementi, ha base (come $\mathbb{F}_2-$spazio vettoriale, l'esponente della $x$ è la dimensione del polinomio):$\{[1],[x]\}$, quindi $\mathbb{F}_4=${insieme di tutte le combinazioni lineari di questa base a coefficienti $\mathbb{F}_2$} = {$a\cdot [1]+b\cdot[x]|(a,b)\in \mathbb{F}_2\times\mathbb{F}_2$} = {$\underset{b=0}{\underset{a=0}{[0]}},\underset{b=0}{\underset{a=1}{[1]}},\underset{b=0}{\underset{a=1}{[x]}},\underset{b=1}{\underset{a=0}{[x+1]}}$} 